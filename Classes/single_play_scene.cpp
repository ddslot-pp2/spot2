...
void single_play_scene::on_http_request_completed(HttpClient *sender, HttpResponse *response) {
    ...
    else if (response->getHttpRequest()->getTag() == std::string("stage-info"))
    {
        ...
        {
            // bool 값이 아닌 enum 으로 세분화해서 error code를 쉽게 볼수 있게해야 할듯
            //CCLOG("recv stage-info but fail to parse json data\n");
        }
...
bool single_play_scene::parsing_stage_info(std::string&& payload)
{
    ...
    if (res["is_max_stage"].bool_value())
    {
...
bool single_play_scene::check_find_answer(const Point& point)
{
    ...
    for (size_t i = 0; i < spots_info_->answer_container.size(); ++i) {
        ...
        if (left_rect.containsPoint(point) || right_rect.containsPoint(point)) {
            ...
            auto audio = SimpleAudioEngine::getInstance();
            audio->playEffect("sound/correct2.wav");
...
void single_play_scene::on_update_timer(float dt)
{
    ...
    progress_timebar_->setPercentage(percentage - (100 / (timer_sec * 10)));
}
...
void single_play_scene::draw_stage_info(int current_stage, int end_stage)
{
    ...
    auto label = Label::createWithTTF(stage_info_font.c_str(), "fonts/Marker Felt.ttf", 36);
    label->setColor(Color3B(255, 255, 255));
    //label->enableOutline(Color4B(255,0,0,255),10);
    //label->setWidth(400);
    label->setPosition(Vec2(center_.x + 290, center_.y + 490));
...
void single_play_scene::draw_spot_info(int found_spot_count, int total_spot_count)
{
    ...
    if (!spot_info_font_) {
        spot_info_font_ = Label::createWithTTF(spot_info_font.c_str(), "fonts/Marker Felt.ttf", 36);
        spot_info_font_->setPosition(Vec2(center_.x + 540, center_.y + 490));
...
void single_play_scene::on_complete_stage(float dt)
{
    ...
    // 일단 완료 했으니 스테이지 저장해주고 승리 효과 주고나서 다음스테이지로 이동
    complete_stage();
}
...
void single_play_scene::complete_stage()
{
    auto win_game = Sprite::create("ui/you_win.png");
    win_game->setPosition(Vec2(center_.x, center_.y + 50));
    this->addChild(win_game, 4);

    win_game->runAction(Sequence::create(Show::create(), FadeOut::create(2.0), nullptr));

    auto audio = SimpleAudioEngine::getInstance();
    audio->playEffect("sound/you_win.wav");

    this->scheduleOnce(schedule_selector(single_play_scene::on_next_stage), 3.0f);   
}

void single_play_scene::on_next_stage(float dt)
{
...
void single_play_scene::create_complete_popup()
{
    auto offset = 0.0;
    ...
    this->addChild(complete_background_popup, 4);

    // windows에서는 한글 안나옴 현상 발생
    ...
    this->addChild(complete_noti_font, 4);
    ...
    this->addChild(complete_confirm_button, 4);

    // RESET 처음부터 다시하기 버튼 추가하기
}

void single_play_scene::end_application()
{
     Director::getInstance()->end();
